...
//播放页
- (void)gotoPlayVCWithSongId:(NSString *)songid {
    PlayViewController *play = [PlayViewController shareSongPlay];
    [play setSongPlayWithID:songid];
...
- (void)gotoPlayVC {
    ...
    NSString *string = [defaults objectForKey:@"songid"];
    if (string.length) {
        [self gotoPlayVCWithSongId:string];
    } else {
...
//歌手页
- (void)gotoArtistVCWithTinguid:(NSString *)tinguid {
    ArtistViewController *artist = [[ArtistViewController alloc] init];
    artist.tinguid = tinguid;
    artist.hidesBottomBarWhenPushed = YES;
